...
@NamedQueries(value = {
		@NamedQuery(name = "CurrencyChartOfAccount.findAll", query = "SELECT c FROM CurrencyChartOfAccount c ORDER BY c.coa.acCode,c.currency.code ASC"),

		@NamedQuery(name = "CurrencyChartOfAccount.findCCOAByCOAid", query = "SELECT c FROM CurrencyChartOfAccount c where c.coa.id=:id"),

		@NamedQuery(name = "CurrencyChartOfAccount.findCOAByBranchID", query = "SELECT DISTINCT NEW org.tech.hms.currencyChartOfAccount.CurrencyChartOfAccount(c.coa) FROM CurrencyChartOfAccount c WHERE c.branch.id=:branchId and c.coa.acCodeType != org.tech.hms.codesetup.AccountCodeType.HEAD AND c.coa.acCodeType != org.tech.hms.codesetup.AccountCodeType.GROUP "),

		@NamedQuery(name = "CurrencyChartOfAccount.findByAcCodeAndBranchId", query = "SELECT c FROM CurrencyChartOfAccount c WHERE c.coa.acCode=:acCode AND c.branch.id=:branchId ORDER BY c.currency.code ASC"),
		...
		// @NamedQuery(name = "CurrencyChartOfAccount.findAllBudget", query = "Select
		// distinct ccoa.budget from CurrencyChartOfAccount ccoa WHERE ccoa.budget IS
		// NOT NULL UNION ALL Select distinct ccoahist.budget from CcoaHistory ccoahist
		// WHERE ccoahist.budget IS NOT NULL"),
		...
		@NamedQuery(name = "CurrencyChartOfAccount.updateByBranchId", query = "UPDATE CurrencyChartOfAccount c SET c.acName=:acName, c.department=:departmentId WHERE c.coa.acCode = :acCode AND c.branch.code = :branchCode") })
